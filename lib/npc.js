...
module.exports = class Npc extends Entity {
    ...
    shoot() {
      if (this.shotDelay == 0 && this.exists) {
        ...
        this.game.soundManager("shot")//new Audio("./sound/shot_sound.wav").play()
      }
    }
    evadeProtocol() {
        this.evade = 0
        if (this.pos.x <= 800 && this.av.x < 0 && this.dv.len() != 0) {
            ...
            this.evade = 1;
        } else if (this.pos.x >= this.game.map.width - 800 && this.av.x > 0 && this.dv.len() != 0) {
            if (Math.atan2(this.av.y, this.av.x) <= 0) {
                this.protocolDirection = 0
            } else {
                this.protocolDirection = 1
            ...
            this.evade = 1;
        } else if (this.pos.y >= this.game.map.height - 800 && this.av.y > 0 && this.dv.len() != 0) {
            if (Math.atan2(this.av.y, this.av.x) <= Math.PI / 2) {
                this.protocolDirection = 0
            } else {
                this.protocolDirection = 1
